...
\title{A novel K nearest neighbor similarity method to detect adversarial samples}
...
\begin{abstract}
	Nowadays, data mining is one of the hot research field, as a huge volume of data is generated everyday by all kinds of applications. The K Nearest Neighbors one of machine learning classification algorithm is widely applied in data mining in pattern recognition, label classification and so on.
	However,despite a lot of work is focused on how to quickly and accurately find the objects we need or the information that benefits us among a large amount of data, there is little work on the classification robustness to the adversarial samples. 
	By adding tiny perturbations to the sample to be classified, that is, disturbances that are too small for the human eye to recognize, the classification algorithm predicts classification results that are distinct from the previous ones totally,the results predicted by the classification algorithm will not be credible, bringing huge threats to the classifiers.
	In this paper,we analyze the representation of data samples in the different feature space,including the data sample dimension reduction method such as GEM and LFD,and the deep learning method to transform the original feature space to low dimension and abstract represention, experiments show that the representation of data samples in different feature spaces will have different robust performance to adversarial samples, finally the improved subspace nearest neighbor similiarity algorithm are used to detect the potential adversarial sample among correctly classified true samples, which can effectively detect these misclassified threatened instace and ensure the reliability of the classifiers.
\end{abstract}

\begin{keywords}
K nearest neighbor,  adversarial machine learning,  data mining
\end{keywords}
...
The k-Nearest neighbor(KNN) is one of the data mining algorithms widely used in many fields that can automatically classify unlabeled sample $x$. It searchs the most similar k neighbors of the unlabeled input sample $x$ in the training data set feature space and assigns the label which the most present among  neighbors to this sample $x$\cite{cover1967nearest}. 
At present, there has been a body of work to optimize the efficiency and accuracy of k-Nearest neighbor and their many varients, e.g., weigh different neighbors specific weights\cite{bicego2016weighted} and dynamic $k$ nearest neighbors in replace of fixed $k$\cite{Zhong:2017:IKC:3055635.3056604}.
All these works however not consider that k-NN is with no sensitive to outliers and no input hypothesis, therefore, adversarial exsample will bring huge threats to the k-NN model.

An Adversarial exsample is a malicious input sample deliberately created applying a small but intentional perturbation, such that the attacked model misclassifies it with high confidence\cite{goodfellow2014explaining}. Especially, more and more k-NN models one of the most common machine learning algorithm or its varients are increasingly applied in artical domains. For example, in the field of autonomous vehicles, someone can intentionally alter a normal danger warning sign slightly,this attack is so negligible to detect for human eye, resultly misclassified to another innoxious sign by the classification model. It is very common in the field of patern recognition but leading to serious result.

Currently, there is a little amount of work focused on the adversarial sample to KNN models.Different from the method proposed in\cite{wang2017analyzing}, it mainly analyzes distributional properties and finite samples on robustness. Building, and proposes a method of r-separated set to resist the attack of the adversarial exsample\cite{wang2017analyzing}. 
However, in this paper, We do research in the other direction, we do not change the state of the original data set as\cite{wang2017analyzing}, but analyze and study on the feature space of the data sample. The current main classification method is to reduce the dimensionality of the data and compress the data into a smaller feature space\cite{lin2011parameter}, so that the data can be more dense and better to represent the original distribution of the data, but there is abother potential problem, after compression, the adversarial samples can be distingished in the original dimensional space will be compressed into "normal" samples in the low dimensional because of singularity disappears, bringing more serious problem. 
So just reducing the dimension is not enough, we need more rigorous and abstract data representation. Moreover, adversarial images are generated in order to look similar to humans, and deep features have shown impressive results in visual similarity related tasks such as content-based image retrieval\cite{gordo2016deep}. Being deep learning a subset of representation learning methods, numerous neurons can transform the original data samples into a more abstract and flexible feature space, and could dig out novel feature representations that are difficult to mine by traditional matrix decomposition methods, we expect the learned representation of the network to be more robust to adversarial examples. 
With the management of feature space, in addition, we perform nearest neighbor similarity search in the subspaces of each class, taking training samples into consideration as many as possible, finally, based on the trade-off consideration and analysis of similarity score of each class to determine the  input sample,whether is a adversarial sample. 
